...
object ModPatterns {
    ...
    // Generic entity name pattern
    val ENTITY_NAME = Regex("""#name\s+"([^"]+)".*$""")
    ...
    val WEAPON_REFERENCES = Regex(
        """#(?:weapon|copyweapon|secondaryeffect|secondaryeffectalways|selectweapon)\s+"([^"]+)".*$"""
    )
    ...
    val ARMOR_REFERENCES = Regex("""#(?:armor|copyarmor|selectarmor)\s+"([^"]+)".*$""")
    ...
    val USE_NUMBERED_ITEM =
        Regex("""#(?:selectitem|startitem|copyitem|req_targitem|req_targnoitem|req_worlditem|req_noworlditem)\s+(\d+)(.*)$""")
    val ITEM_REFERENCES = Regex("""#(?:selectitem|startitem|copyitem|req_targitem|req_targnoitem|req_worlditem|req_noworlditem)\s+"([^"]+)".*$""")

    // Sprite copy (context-sensitive): per the modding manual, #copyspr exists for BOTH
    // monsters ("#copyspr <monster nbr>", inside a #newmonster/#selectmonster block) and
    // items ("#copyspr <item nbr>", inside a #newitem/#selectitem block). It is therefore
    // deliberately kept OUT of USE_MONSTER and USE_NUMBERED_ITEM; EntityProcessor resolves
    // the target entity type from the enclosing definition block. Numeric only.
    val USE_COPYSPR = Regex("""#copyspr\s+(\d+)(.*)$""")
    ...
    val SITE_REFERENCES = Regex("""#(?:selectsite|godsite|onlyatsite|onlysitedst)\s+"([^"]+)".*$""")
    ...
    val NATION_REFERENCES = Regex("""#(?:nation|restricted|notfornation|nationrebate|req_nation|req_nonation|req_fornation|req_notfornation|req_notnation|req_notforally|req_fullowner|req_domowner|req_targowner|assowner|extramsg|nat|req_targnotowner)\s+"([^"]+)".*$""")
